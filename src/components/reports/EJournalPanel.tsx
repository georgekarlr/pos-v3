...
const EJournalPanel: React.FC = () => {
  ...
  const formatFullDateTime = (dateStr: string | Date) => {
    try {
  ...
  const generateEJournalText = useCallback(() => {
    ...
    text += `Generated On: ${formatFullDateTime(new Date())}\n`
    text += `${starLine}\n\n`
    ...
    filtered.forEach(row => {
      ...
      text += `Date/Time: ${formatFullDateTime(row.created_at)}\n`
      text += `Terminal: ${row.terminal_name}\n`
      
      const detailsObj = row.details || {}
      const accountId = detailsObj.account_id || detailsObj.user_id || 'NULL'
      text += `User: ${row.account_name} (ID: ${accountId})\n`
      ...
      if (row.event_type === 'SALE') {
        ...
        text += `Customer: ${customerName} (ID: ${customerId})\n\n`
        ...
        if (items.length > 0) {
          ...
          items.forEach((item: any, index: number) => {
            ...
            const price = Number(item.price || item.price_at_purchase || 0).toFixed(2)
            const taxType = item.tax_type === 'VATable' || item.tax_type_at_purchase === 'VATable' ? 'VATable' : 'VAT-Exempt'
            const total = Number(item.line_total || item.total || (Number(qtyStr) * Number(price))).toFixed(2)
        ...
        text += `  Discounts:           ${Number(detailsObj.discounts || detailsObj.discount_amount || 0).toFixed(2).padStart(10)}\n`
        text += `  Net Amount:          ${Number(detailsObj.net_amount || detailsObj.total_amount || detailsObj.total || 0).toFixed(2).padStart(10)}\n`
        
        const vat = detailsObj.vat || {}
        text += `  VATable Sales:       ${Number(vat.vatable_sales || vat.vatable || detailsObj.vatable_sales || 0).toFixed(2).padStart(10)}\n`
        text += `  VAT Amount:          ${Number(vat.vat_amount || vat.vat_amount_12 || detailsObj.vat_amount || 0).toFixed(2).padStart(10)}\n`
        text += `  VAT-Exempt:          ${Number(vat.vat_exempt_sales || vat.exempt || detailsObj.vat_exempt_sales || 0).toFixed(2).padStart(10)}\n\n`
