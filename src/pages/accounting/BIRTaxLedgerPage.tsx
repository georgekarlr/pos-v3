...
import { BIRTaxLedgerResult } from '../../types/accounting'
...
export const BIRTaxLedgerPage: React.FC = () => {
  ...
  const [data, setData] = useState<BIRTaxLedgerResult | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await AccountingService.getBIRTaxLedger({
      ...
      console.error('Failed to load BIR tax ledger:', err)
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Automated BIR Form 2550Q (VAT) & BIR Form 2551Q (Percentage Tax) returns generator.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <TaxLedgerView data={data} startDate={startDate} endDate={endDate} />
      )}
    </div>
  )
}
