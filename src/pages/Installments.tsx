  ...
  const handleCreateSale = async (params: Omit<CreateInstallmentSaleParams, 'p_account_id'> & {
    ...
    if (result.data && result.data.success) {
      ...
      const rData: ReceiptData = {
        ...
        lines: (params.p_cart_full || []).map((l: any) => ({
          name: l.name,
          qty: l.qty,
          unitType: l.unit_type || l.unitType,
          unitPrice: l.unitPrice ?? l.display_price ?? l.base_price ?? 0,
          baseUnitPrice: l.base_price ?? l.baseUnitPrice,
          lineTotal: l.lineTotal ?? l.displayLineTotal ?? 0,
          taxType: l.tax_type || l.taxType || 'VATable',
          isScPwdEligible: l.is_sc_pwd_eligible || l.isScPwdEligible,
